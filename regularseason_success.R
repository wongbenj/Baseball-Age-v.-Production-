...
batting <- read.csv("C:/Users/Benjamin Wong/Desktop/Batting.csv")
...
batting_new <- batting[ which(batting$yearID == 2016), ]

# aggregate(list(batting_new$G, batting_new$AB, batting_new$R, batting_new$H, batting_new$X2B, batting_new$X3B, batting_new$HR, batting_new$RBI, batting_new$SB, batting_new$CS, batting_new$BB, batting_new$SO, batting_new$IBB, batting_new$HBP, batting_new$SH, batting_new$SF, batting_new$GIDP), by = list(batting_new$playerID), sum)

# Merge the master dataset with the postseason batting dataset
merge_set <- merge(batting_new, master, by="playerID")
...
plot_age <- agg_merge_set$age
plot_wOBA <- agg_merge_set$wOBA
...
ggplot(agg_merge_set, aes(x=age, y=wOBA)) + 
  ...
  labs(title="MLB: Age vs. wOBA", subtitle="2016 Regular Season", y="wOBA", x="Player Age")
...
ggplot(agg_merge_set, aes(x=experience, y=wOBA)) + 
  ...
  # Add Title and Labels
  labs(title="MLB: Years of Experience vs. wOBA", subtitle="2016 Regular Season", y="wOBA", x="Player's Experience (years)")
...
ggplot(agg_merge_set, aes(x=age, y=wOBA)) + 
  ...
  scale_y_continuous(name = "wOBA",
                     breaks = seq(0, 0.9, 0.20),
                     limits=c(0,0.9)) +
  scale_x_discrete(name = "Player Age") +
  ggtitle("Boxplot of mean wOBA by age for 2016 Regular Season")
...
ggplot(agg_merge_set_mean, aes(x=age, y=wOBA)) +
  ...
  # Delete the points outside the limits
  #xlim(c(19, 41)) + ylim(c(0.05, 0.4)) +   # deletes points

  # Add Title and Labels
  labs(title="MLB: Age vs. Mean wOBA", subtitle="2016 Regular Season", y="wOBA", x="Player Age")
